...
function BColorMap(elname, formname) {
	...
	for (var i=0; i<colors.length; i++) {
		if (formname == undefined) {
			html += "<td style='width:10px;height:10px;cursor:crosshair;background-color:" + colors[i] + "' onclick=\"addText('" + elname + "', '[bcolor=" + colors[i] + "]', '[/bcolor]');return false;\" onfocus=\"showMapBColor('" + colors[i] +  "', '" + elname + "');\" onmouseover=\"showMapBColor('" + colors[i] + "', '" + elname + "');\">"
		} else {
			html += "<td style='width:10px;height:10px;cursor:crosshair;background-color:" + colors[i] + "' onclick=\"addText('" + elname + "', '[bcolor=" + colors[i] + "]', '[/bcolor]', '" + formname + "');return false;\" onfocus=\"showMapBColor('" + colors[i] +  "', '" + elname + "');\" onmouseover=\"showMapBColor('" + colors[i] + "', '" + elname + "');\">"
		}
		html += '</td>';
