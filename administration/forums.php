...
if (isset($_POST['save_cat'])) {
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['forum_id']) && isnum($_GET['forum_id'])) && (isset($_GET['t']) && $_GET['t'] == "forum")) {
		...
		if ($forum_cat != dbquery("SELECT forum_cat FROM ".DB_FORUMS." WHERE forum_id='$forum_id'")) {
			$forum_order = dbresult(dbquery("SELECT MAX(forum_order) FROM ".DB_FORUMS." WHERE forum_cat='$forum_cat'"),0)+1;
		} else {
			$forum_order = dbquery("SELECT forum_order FROM ".DB_FORUMS." WHERE forum_id='$forum_id'");
		}
