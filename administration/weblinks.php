...
$result = dbquery("SELECT * FROM ".DB_WEBLINK_CATS."");
if (dbrows($result)) {
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['weblink_id']) && isnum($_GET['weblink_id']))) {
		$result = dbquery("SELECT * FROM ".DB_WEBLINKS." WHERE weblink_id='".$_GET['weblink_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			...
			$formaction = FUSION_SELF.$aidlink."&amp;action=edit&amp;weblink_id=".$data['weblink_id'];
	...
	$result = dbquery("SELECT * FROM ".DB_WEBLINK_CATS." ORDER BY weblink_cat_name");
	if (dbrows($result)) {
		echo "<tr>\n";
		...
		while ($data = dbarray($result)) {
			...
			echo "</tr>\n";
			$result2 = dbquery("SELECT * FROM ".DB_WEBLINKS." WHERE weblink_cat='".$data['weblink_cat_id']."' ORDER BY weblink_name");
			if (dbrows($result2)) {
				echo "<tr>\n<td colspan='2'>\n";
