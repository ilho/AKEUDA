...
if ($settings['forum_ranks']) {
	if (isset($_POST['save_rank'])) {
		...
		$rank_apply = isnum($_POST['rank_apply']) ? $_POST['rank_apply'] : 101;
		if ($rank_title) {
			if (isset($_GET['rank_id']) && isnum($_GET['rank_id'])) {
				...
				if (($rank_apply > 101 && $rank_apply != $data['rank_apply']) && (dbcount("(rank_id)", DB_FORUM_RANKS, "rank_id!='".$_GET['rank_id']."' AND rank_apply='".$rank_apply."'"))) {
					...
					$result = dbquery("UPDATE ".DB_FORUM_RANKS." SET rank_title='$rank_title', rank_image='$rank_image', rank_posts='$rank_posts', rank_apply='$rank_apply' WHERE rank_id='".$_GET['rank_id']."'");
				...
				if ($rank_apply > 101 && dbcount("(rank_id)", DB_FORUM_RANKS, "rank_apply='".$rank_apply."'")) {
					...
					$result = dbquery("INSERT INTO ".DB_FORUM_RANKS." (rank_title, rank_image, rank_posts, rank_apply) VALUES ('$rank_title', '$rank_image', '$rank_posts', '$rank_apply')");
	...
	if (isset($_GET['rank_id']) && isnum($_GET['rank_id'])) {
		$result = dbquery("SELECT * FROM ".DB_FORUM_RANKS." WHERE rank_id='".$_GET['rank_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
	...
	$result = dbquery("SELECT * FROM ".DB_FORUM_RANKS." ORDER BY rank_apply DESC, rank_posts");
	if (dbrows($result)) {
		...
		while ($data = dbarray($result)) {
			...
			if ($data['rank_apply'] == 101) {
				echo $locale['424'];
			} elseif ($data['rank_apply'] == 102) {
				echo $locale['426'];
			} elseif ($data['rank_apply'] == 103) {
				echo $locale['427'];
			} elseif ($data['rank_apply'] == 104) {
				echo $locale['425'];
			}
			echo "</td>\n";
