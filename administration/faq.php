...
if (!checkrights("FQ") || !defined("iAUTH") || $_GET['aid'] != iAUTH) { redirect("../index.php"); }
...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['faq_cat_id']) && isnum($_GET['faq_cat_id'])) && (isset($_GET['t']) && $_GET['t'] == "cat")) {
	$result = dbquery("SELECT * FROM ".DB_FAQS." WHERE faq_cat_id='".$_GET['faq_cat_id']."'");
	if (dbrows($result) != 0) {
	...
	if ((isset($_GET['faq_cat_id']) && isnum($_GET['faq_cat_id'])) && (isset($_GET['t']) && $_GET['t'] == "cat")) {
		$result = dbquery("SELECT * FROM ".DB_FAQ_CATS." WHERE faq_cat_id='".$_GET['faq_cat_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
		...
		$result = dbquery("SELECT * FROM ".DB_FAQS." WHERE faq_id='".$_GET['faq_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			$faq_cat_name = "";
			$faq_cat_description = "";
...
if (!isset($_GET['t']) || $_GET['t'] != "cat") {
	$cat_opts = ""; $sel = "";
	$result2 = dbquery("SELECT * FROM ".DB_FAQ_CATS." ORDER BY faq_cat_name");
	if (dbrows($result2) != 0) {
...
if (dbrows($result) != 0) {
	...
	while ($data = dbarray($result)) {
		...
		echo "</tr>\n";
		$result2 = dbquery("SELECT * FROM ".DB_FAQS." WHERE faq_cat_id='".$data['faq_cat_id']."' ORDER BY faq_id");
		if (dbrows($result2) != 0) {
			echo "<tr>\n<td colspan='2'>\n";
