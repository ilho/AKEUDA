...
if (!checkrights("DC") || !defined("iAUTH") || $_GET['aid'] != iAUTH) redirect("../index.php");
...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	$result = dbquery("SELECT * FROM ".DB_DOWNLOADS." WHERE download_cat='".$_GET['cat_id']."'");
	if (dbrows($result) != 0) {
	...
	if (dbrows($result) != 0) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<tr>\n";
			echo "<td class='".$cell_color."'>".$data['download_cat_name']."<br />\n";
			echo "<span class='small'>".trimlink($data['download_cat_description'], 45)."</span></td>\n";
			echo "<td align='center' width='1%' class='$cell_color' style='white-space:nowrap'>".getgroupname($data['download_cat_access'])."</td>\n";
