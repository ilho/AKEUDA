...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	$result = dbquery("SELECT * FROM ".DB_ARTICLES." WHERE article_cat='".$_GET['cat_id']."'");
	if (dbrows($result) != 0) {
	...
	if (dbrows($result) != 0) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<tr>\n";
			echo "<td class='$cell_color'><strong>".$data['article_cat_name']."</strong><br />\n";
			echo "<span class='small'>".trimlink($data['article_cat_description'], 45)."</span></td>\n";
