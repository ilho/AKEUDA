...
if (isset($_POST['previewchanges']) || isset($_POST['update_poll_title']) || isset($_POST['update_poll_option']) || isset($_POST['delete_poll_option']) || isset($_POST['add_poll_option'])) {
	...
	$subject = isset($_POST['subject']) ? trim(stripinput(censorwords($_POST['subject']))) : "";
			...
			if (isset($_POST['poll_options']) && is_array($_POST['poll_options'])) {
				...
				foreach ($_POST['poll_options'] as $poll_option) {
					if (isset($_POST['delete_poll_option'][$i])) {
						$data = dbarray(dbquery("SELECT * FROM ".DB_FORUM_POLL_OPTIONS." WHERE thread_id='".$_GET['thread_id']."' AND forum_poll_option_id='".$i."'"));
	...
	if (isset($_POST['previewchanges'])) {
		...
		$udata = dbarray(dbquery("SELECT * FROM ".DB_USERS." WHERE user_id='".$pdata['post_author']."'"));
		...
		echo "<tr>\n<td class='tbl2' style='width:140px;'><a href='../profile.php?lookup=".$udata['user_id']."'>".$udata['user_name']."</a></td>\n";
		...
		echo "<br /></td>\n<td valign='top' class='tbl1'>".$previewmessage;
		echo "<hr />\n".$locale['427']."<a href='../profile.php?lookup=".$userdata['user_id']."'>".$userdata['user_name']."</a>".$locale['428'].showdate("forumdate", time())."</td>\n";
		echo "</tr>\n</table>\n";
		closetable();
	}
}
if (isset($_POST['savechanges'])) {
	if (isset($_POST['delete'])) {
		...
		if (dbrows($result)) {
			...
			if ($posts) {
				$result = dbquery("SELECT * FROM ".DB_THREADS." WHERE thread_id='".$_GET['thread_id']."' AND thread_lastpost='".$pdata['post_datestamp']."' AND thread_lastuser='".$pdata['post_author']."'");
				if (dbrows($result)) {
		...
		if (iMEMBER) {
			if ($message != "") {
				...
				//$data = dbarray(dbquery("SELECT * FROM ".DB_POSTS." WHERE thread_id='".$_GET['thread_id']."' ORDER BY post_id ASC LIMIT 1"));
				...
				if (isset($_POST['delete_attach'])) {
					...
					if (dbrows($result) != 0) {
						...
						$result = dbquery("DELETE FROM ".DB_FORUM_ATTACHMENTS." WHERE post_id='".$_GET['post_id']."'");
					}
				}
	...
	if (!isset($_POST['previewchanges']) && !isset($_POST['update_poll_title']) && !isset($_POST['update_poll_option']) && !isset($_POST['delete_poll_option']) && !isset($_POST['add_poll_option'])) {
		...
		$del_check = "";
		if ($pdata['post_author'] == $tdata['thread_author'] && $tdata['thread_poll']) {
			$result = dbquery("SELECT * FROM ".DB_FORUM_POLLS." WHERE thread_id='".$_GET['thread_id']."'");
			if (dbrows($result)) {
				$data = dbarray($result);
				$poll_title = $data['forum_poll_title'];
				$result = dbquery("SELECT * FROM ".DB_FORUM_POLL_OPTIONS." WHERE thread_id='".$_GET['thread_id']."' ORDER BY forum_poll_option_id ASC");
	...
	if (!isset($_POST['previewchanges'])) echo "<div class='tbl2' style='margin-bottom:5px'><span class='small'><a href='index.php'>".$settings['sitename']."</a> :: ".$caption."</span></div>\n";
	...
	echo "<td class='tbl1'>\n";
	echo "<label><input type='checkbox' name='disable_smileys' value='1'".$disable_smileys_check." /> ".$locale['482']."<br /></label>\n";
	...
	if ($fdata['forum_attach'] && checkgroup($fdata['forum_attach'])) {
		...
		if (dbrows($result)) {
			$adata = dbarray($result);
			echo "<label><input type='checkbox' name='delete_attach' value='1' /> ".$locale['485']."</label>\n";
			...
			echo "<input type='file' name='attach' class='textbox' style='width:200px;' /><br />\n";
	...
	if ($fdata['forum_poll'] && checkgroup($fdata['forum_poll'])) {
		if ($tdata['thread_poll'] && ($pdata['post_author'] == $tdata['thread_author']) && ($userdata['user_id'] == $tdata['thread_author'] || iSUPERADMIN || iMOD)) {
			...
			echo "<input type='submit' name='update_poll_title' value='".$locale['473']."' class='button' />\n</td>\n</tr>\n";
			$i = 1;
